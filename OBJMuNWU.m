...
%     K(2,:) = (1-Para.delta_K)*K(1,:) + S(1,:);%.*Ltol(1,:);%这里s可以作为控制变量(这里的S已经是总储蓄了，因为前面的Q已经是总GDP，所以不该×L）
    ...
    Lambda(1,:) = AbateCost(mu(1,:),Var.theta1(1,:),Para.theta(2,1));
    
    Y(1,:) = (1-Lambda(1,:))./(1.+D(1,:)).*Q(1,:);
    S(1,:) = Y(1,:).*s(1,:);
    K(2,:) = (1-Para.delta_K)*K(1,:) + S(1,:);
...
%         K(t+1,:) = (1-Para.delta_K)*K(t,:) + S(t,:);%.*Ltol(t,:);
        ...
        Lambda(t,:) = AbateCost(mu(t,:),Var.theta1(t,:),Para.theta(2,1));
        
        Y(t,:) = (1-Lambda(t,:))./(1.+D(t,:)).*Q(t,:);
        S(t,:) = Y(t,:).*s(t,:);
        K(t+1,:) = (1-Para.delta_K)*K(t,:) + S(t,:);
    end
    K(size(K,1),:) = [];
    C = (Y-S)./Ltol;
    ...
    for t = 1:Tmax
        ...
        Ybar(t,:,:) = bsxfun(@times,Y(t,:)./(Ltol(t,:)/5),q);
...
%     NegishiW(NegishiW>1) = 1;
    ...
    wel = times(R,ones(1,5,12)).*U.*NegishiW;
